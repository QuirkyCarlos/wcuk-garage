...
$(document).on("click", "#spawncar", function () {
  ...
  if (laststored == '0') {
    ...
    $.post('https://wcuk-garage/spawnvehicle', JSON.stringify({ plate: lastplate , model : lastmodel}));
    $('.container').css('display', 'none')
    $.post('https://wcuk-garage/closepage', JSON.stringify({}));
    lastplate = null;
...
$(document).on("click", ".transfertovehicle", function () {
  ...
  if (!isNaN(id) && id != '') {
    if (typeof (lastplate != 'undefined')) {
      $.post('https://wcuk-garage/transfervehicle', JSON.stringify({plate: lastplate,id: id }));
      $('.container').css('display', 'none')
      $.post('https://wcuk-garage/closepage', JSON.stringify({}));
      lastplate = null;
      document.querySelectorAll(".vehiclemodel").forEach(function (a) { a.remove() })
...
$(document).on("click", ".pricebutton", function () {
  ...
  $.post('https://wcuk-garage/sellvehicle', JSON.stringify({
    plate: lastplate,
    hash: lasthash,
    price: lastprice
  }), function (sell) {
