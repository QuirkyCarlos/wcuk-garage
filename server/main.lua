...
Citizen.CreateThread(function()
	...
	if Config.Framework == 'esx' then	
	frameworkObject.RegisterServerCallback('wcuk-garage:loadvehicles2', function(source, cb)	
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
		frameworkObject.RegisterServerCallback('wcuk-garage:loadvehicles', function(source, cb)
		local ownedCars = {}
		...
		frameworkObject.Functions.CreateCallback('wcuk-garage:loadvehicles', function(source, cb)
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
		frameworkObject.RegisterServerCallback('wcuk-garage:impoundGarage', function(source, cb)
		local impoundCars = {}
		...
		frameworkObject.Functions.CreateCallback('wcuk-garage:impoundGarage', function(source, cb)
			local impoundCars = {}
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
		frameworkObject.RegisterServerCallback('wcuk-garage:vehicleOwner', function(source,cb,plate)
		local cars = ExecuteSql("SELECT * FROM owned_vehicles WHERE `plate` ='"..plate.."'")	
		cb(cars)
		end)
	else -- qbcore
		frameworkObject.Functions.CreateCallback('wcuk-garage:vehicleOwner', function(source,cb,plate)
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
		frameworkObject.RegisterServerCallback('wcuk-garage:vehicleOwned', function(source, cb, plate)
		local xPlayer = GetIdentifier(source)	
		...
		frameworkObject.Functions.CreateCallback('wcuk-garage:vehicleOwned', function(source, cb, plate)
			local Player = frameworkObject.Functions.GetPlayer(source)
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
	    frameworkObject.RegisterServerCallback('wcuk-garage:changedCars', function(source, cb, plate)
		...
		frameworkObject.Functions.CreateCallback('wcuk-garage:changedCars', function(source, cb, plate)
...
RegisterNetEvent('wcuk-garage:saveProps')
AddEventHandler('wcuk-garage:saveProps', function(plate, props)
...
RegisterServerEvent('wcuk-garage:sellPrice')
AddEventHandler('wcuk-garage:sellPrice', function(plate,price)
...
RegisterServerEvent('wcuk-garage:TransferVehicle')
AddEventHandler('wcuk-garage:TransferVehicle', function(id,plate)
		...
		if xTarget ~= nil then		
			if id ~= src then
				...
				if devret then
					...
					if Config.TransferWebhook ~= '' then					
						dclog(Config.TransferWebhook, "Garage Transfer LOG","\n Vehicle Owner \n Discord : " ..id2.."\n Steam : " ..steam.."\n ID : " ..src.."\n Vehicle Plate : " ..plate.."\n  New Vehicle Owner \n   Discord : " ..pid.."\n Steam : " ..steam2.."\n ID : " ..id.."\n Vehicle Plate : " ..plate.." ")
					end
				end
			else
...
Citizen.CreateThread(function()
	frameworkObject = GetFrameworkObject()
	if Config.Framework == 'esx' then
	frameworkObject.RegisterServerCallback('wcuk-garage:checkMoneyCars', function(source, cb,garajtype)
		...
		frameworkObject.Functions.CreateCallback('wcuk-garage:checkMoneyCars', function(source, cb,garajtype)
			local src = source
...
RegisterServerEvent('wcuk-garage:payCar')
AddEventHandler('wcuk-garage:payCar', function(par1)
...
RegisterNetEvent('wcuk-garage:stored')
AddEventHandler('wcuk-garage:stored', function(plate, state)
	...
	local connect = {
	    {
	        ...
	        ["description"] = text,
	        ["footer"] = {
		  ["text"] = "WILDCARLUK" ..time,
		  ["icon_url"] = Config.IconURL,
	        },
	    }
	}
	PerformHttpRequest(Config.TransferWebhook, function(err, text, headers) end, 'POST', json.encode({username = Config.Botname, embeds = connect, avatar_url = Config.Logo}), { ['Content-Type'] = 'application/json' })
end
